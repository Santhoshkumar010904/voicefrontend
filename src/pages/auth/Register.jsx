...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Registration successful! Redirecting...', { duration: 5000 });
        localStorage.setItem('userInfo', JSON.stringify(data));
        // Simulate redirect logic after a slight delay
        setTimeout(() => navigate('/connect-gmail'), 2000);
      } else {
  ...
  return (
    <div className="h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-row items-center justify-center w-full max-w-[1250px] mx-auto px-8 z-10 gap-16 h-full">
        ...
        >
          {/* Subtle Background Orbs for the left side */}
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8 lg:p-10 max-h-full overflow-y-auto overflow-x-hidden scrollbar-hide relative">
            ...
            <div className="flex items-center gap-4 mb-8 pr-10">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-1">Join VoiceBridge and experience the power of voice.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="flex gap-4">
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="relative">
                    <BiUser className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input 
                      type="text" 
                      name="name"
                      value={formData.name}
                      ...
                      className={`w-full bg-slate-50 border text-sm rounded-xl pl-10 pr-4 py-2.5 outline-none focus:bg-white transition-all text-slate-700 placeholder:text-slate-400 font-medium ${errors.name ? 'border-red-400 focus:border-red-500' : 'border-slate-200 focus:border-indigo-500'}`} 
                    />
                  </div>
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 border text-sm rounded-xl pl-10 pr-4 py-2.5 outline-none focus:bg-white transition-all text-slate-700 placeholder:text-slate-400 font-medium ${errors.email ? 'border-red-400 focus:border-red-500' : 'border-slate-200 focus:border-indigo-500'}`} 
                    />
                  </div>
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 border text-sm rounded-xl pl-10 pr-10 py-2.5 outline-none focus:bg-white transition-all text-slate-700 placeholder:text-slate-400 font-medium ${errors.password ? 'border-red-400 focus:border-red-500' : 'border-slate-200 focus:border-indigo-500'}`} 
                    />
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 border text-sm rounded-xl pl-10 pr-10 py-2.5 outline-none focus:bg-white transition-all text-slate-700 placeholder:text-slate-400 font-medium ${errors.confirmPassword ? 'border-red-400 focus:border-red-500' : 'border-slate-200 focus:border-indigo-500'}`} 
                    />
                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                      {showConfirmPassword ? <BiShow size={18} /> : <BiHide size={18} />}
                    </button>
                  </div>
              ...
              {/* Submit Button */}
              <button type="submit" className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold text-sm py-3 rounded-xl transition-all shadow-md shadow-indigo-200 hover:shadow-lg flex items-center justify-center gap-2 mt-2">
                Create Account
              ...
              {/* Google Register */}
              <button type="button" className="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-sm py-3 rounded-xl transition-all shadow-sm flex items-center justify-center gap-2">
                <FcGoogle size={18} />
              ...
              <div className="text-center pt-2">
                <p className="text-xs text-slate-500 font-medium">
                  Already have an account? <button type="button" onClick={() => navigate('/login')} className="text-indigo-600 font-bold hover:underline">Login</button>
                </p>
              </div>
